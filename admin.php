	...
	/* if ($_FILES["background_image"]["error"] > 0)
  {
   ...
   echo 'file name1'.$_FILES["background_image"]; */
	...
	if($res_get_shop && pg_num_rows($res_get_shop) > 0){ // store settings already exists (update them)
		...
		$sql = "INSERT INTO tbl_usersettings (heading, access_token,store_name,message_text, button1_text, button1_color, button2_text, button2_color, background_image, cookie_lifetime) values ('$Heading_text', '$sid','$store', '$message_text', '$Button1_text', '$Button1_color', '$Button2_text', '$Button2_color', '$background_image', '$cookie_lifetime')";
	}
